...
| `externalId` | string | No | An external unique ID for the user. Use it to associate a user in another system, such as a student or employee ID number. |
...
| `onPremisesSamAccountName` | string | No | Contains the on-premises SAM account name synchronized from the on-premises directory. Read-only. |
...
| `primaryRole` | string | No | The user`s default role. Such as "student" or "teacher" |
